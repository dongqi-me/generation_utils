...
class Dataset(object):
    ...
    def __init__(self, file = None, vocab_file = None):
    ...
    def iterate_once_doc_bert(self):
        ...
        for value in self._doc_iterate_bert(self._doc_stream_bert(self.file)):
            yield value

    def _doc_stream_bert(self, file):
        
        for doc in self._parse_file2doc_bert(file):
            yield doc

    def _parse_file2doc_bert(self, file):
        print("Processing file: %s" % file)
        with open(file,'r') as f:
            data = json.load(f)
        for obj in data:
            
            article, abstract = nltk.tokenize.sent_tokenize(obj['Paper_Body']), obj['News_Body']
